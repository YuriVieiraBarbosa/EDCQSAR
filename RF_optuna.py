...
OUTPUT_DIR = "/home/yuri/Projetos/EDCQSAR/RF"
...
print("\nTreinando Random Forest com melhores parâmetros...")
...
importance.to_csv(
    os.path.join(OUTPUT_DIR, "RF_hybrid_importance.csv"),
    index=False
)
...
best_params_df.to_csv(
    os.path.join(OUTPUT_DIR, "optuna_rf_best_params.csv"),
    index=False
)

trials_df = study.trials_dataframe()
trials_df.to_csv(
    os.path.join(OUTPUT_DIR, "optuna_rf_trials.csv"),
    index=False
)
...
plt.title(f"Random Forest + Optuna (R²={r2:.3f})")
plt.tight_layout()
plt.savefig(os.path.join(OUTPUT_DIR, "RF_hybrid_real_vs_predito.png"), dpi=300)
plt.close()
...
plt.tight_layout()
plt.savefig(os.path.join(OUTPUT_DIR, "RF_hybrid_histograma_erros.png"), dpi=300)
plt.close()
...
plt.title("Resíduos")
plt.tight_layout()
plt.savefig(os.path.join(OUTPUT_DIR, "RF_hybrid_residuos.png"), dpi=300)
plt.close()
...
plt.tight_layout()
plt.savefig(os.path.join(OUTPUT_DIR, "RF_hybrid_top20_importance.png"), dpi=300)
plt.close()

# =====================================================
# GIF - EVOLUÇÃO DO FIT COM N_ESTIMATORS
# =====================================================
...
frames_dir = os.path.join(OUTPUT_DIR, "frames_rf_gif")
...
frame_indices = np.unique(
    np.linspace(10, n_estimators_final, num=min(30, n_estimators_final), dtype=int)
)
...
for step in frame_indices:
    ...
    step_params["n_estimators"] = int(step)
    step_params["warm_start"] = False
    step_params["random_state"] = RANDOM_STATE
    step_params["n_jobs"] = -1

    model_step = RandomForestRegressor(**step_params)
    model_step.fit(X_train, y_train)

    pred_step = model_step.predict(X_test)
    ...
    fig.suptitle("Evolução do aprendizado do Random Forest", fontsize=14)
    fig.tight_layout()
...
imageio.mimsave(
    os.path.join(OUTPUT_DIR, "RF_hybrid_learning.gif"),
    gif_frames,
    duration=0.4
)
